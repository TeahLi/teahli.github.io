...
原画（`assets/ink-canyon.jpg`）在纵向 86% 处本来就画了一条船。设计稿的取景只显示
画面的 45.5%~85.5%，那条船在裁切线以下看不到；但竖屏手机为了铺满高度会一路显示到
94%，画里的船就会和 JS 画的那条一起出现，变成两条船。

`site.js` 里的 `IMG_CUT = 0.85` 就是干这个的：只取原图上方 85%，下方不足的部分
用水面最后几行向下延展补齐。换背景图时如果新图没有这个问题，把它改成 `1.0` 即可。
